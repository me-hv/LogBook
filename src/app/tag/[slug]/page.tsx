...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `#${tag.name} | LogBook Tags`,
    description: `Articles tagged with #${tag.name} on LogBook.`,
  };
}
...
export default async function TagPage({ params, searchParams }: PageProps) {
  ...
  // Query posts containing the given tag
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 space-y-8">
      ...
      <PostList
        ...
        fallbackText={`No articles found tagged with "#${tag.name}".`}
      />
